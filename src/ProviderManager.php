...
use Exception;
use App\template\EmailTemplateManager;
...
// Specify the log file path
    public $logFile = __DIR__ . '/emailLog.txt';
...
// Initialize Services
    ...
    public function sendEmail(string $to, array $data, string $templateName): bool
    {
        ...
        //If template is not exists, add Exception
        if (!$template) {
        ...
        while ($failCount < $maxRetries) {
            ...
            try {
                ...
                if ($provider[1]($to, $template['subject'], $template['body'])) {
                    echo "Email sent successfully via {$provider[0]}<br/>";
                    ...
                    echo "{$provider[0]} failed. Retrying...<br/>";
                }
            } catch (Exception $e) {
                echo "{$provider[0]} threw an exception: " . $e->getMessage() . "<br/>";
            }

            $failCount++;
        }
        //Add failure message in log file
        file_put_contents($this->logFile, date('Y-m-d H:i:s') . " : {$template['subject']} mail sent failed" . PHP_EOL, FILE_APPEND);
        echo "All email providers failed after $maxRetries retries.<br/>";
        return false;
    }
}
